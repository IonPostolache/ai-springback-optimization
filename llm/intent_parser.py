...
def parse_requirement(requirement_text: str) -> EngineeringSpec:
    ...
    Import of the LLM client is deliberately lazy (inside this function,
    not at module level) so that EngineeringSpec and the fallback path in
    run_demo.py remain importable and usable even if the `openai` package
    or LM Studio backend isn't available -- only calling this function
    requires a live LLM connection.
    """
