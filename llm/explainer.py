...
def explain_result(search_result: dict, final_evaluation: dict) -> str:
    ...
    Import of the LLM client is deliberately lazy (see intent_parser.py
    for the same pattern) so this module stays importable without a live
    LLM connection -- only calling this function requires one.
    """
...
if __name__ == "__main__":
    example_search_result = {
        "status": "converged",
        "r_max_feasible": 5.696,
        "bracket_width_mm": 0.032,
        "iterations": 9,
    }
    example_evaluation = {
